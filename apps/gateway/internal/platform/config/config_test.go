...
func TestLoadRejectsInsecureProductionTelemetry(t *testing.T) {
	values := mapSource{"GATEWAY_ENVIRONMENT": "production", "GATEWAY_SERVICE_CLIENT_ID": "gateway", "GATEWAY_SERVICE_CLIENT_SECRET": "secret", "GATEWAY_IDENTITY_URL": "https://identity.example", "GATEWAY_ORDERS_URL": "https://orders.example", "GATEWAY_JWKS_URL": "https://identity.example/.well-known/jwks.json", "GATEWAY_REDIS_URL": "rediss://redis.example:6380/1", "GATEWAY_OTEL_ENDPOINT": "otel.example:4317", "GATEWAY_OTEL_INSECURE": "true"}
	if _, err := Load(values); err == nil {
		t.Fatal("insecure production telemetry accepted")
	}
}

func TestLoadRejectsInvalidConfiguration(t *testing.T) {
	for _, values := range []mapSource{{"GATEWAY_ENVIRONMENT": "invalid"}, {"GATEWAY_MAX_RESPONSE_BYTES": "0"}, {"GATEWAY_CORS_ORIGINS": "*"}, {"GATEWAY_TRUST_PROXY_HEADERS": "maybe"}, {"GATEWAY_METRICS_PATH": "/internal-metrics"}} {
		if _, err := Load(values); err == nil {
