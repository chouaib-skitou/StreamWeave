...
func renderIdentityEmailWithSupport(kind, recipient, token, publicAppURL, verificationPath, resetPath, supportEmail string) (emailContent, error) {
	...
	if kind == "email-verification" {
		...
		return emailContent{
			Subject: "Verify your StreamWeave account",
			Text:    renderText(name, "Verify your account", "Confirm your email address to activate your StreamWeave account.", link, "This link expires in 24 hours and can be used only once. If you did not create this account, you can safely ignore this message.", supportEmail),
			HTML:    renderHTML(name, "Verify your account", "Confirm your email address to activate your StreamWeave account.", "Verify email address", link, "This link expires in 24 hours and can be used only once. If you did not create this account, you can safely ignore this message.", supportEmail),
		}, nil
	}
	if kind == "password-reset" {
		link := actionURL(publicAppURL, resetPath, token)
		return emailContent{
			Subject: "Reset your StreamWeave password",
...
func renderText(name, title, intro, link, note, supportEmail string) string {
	return strings.Join([]string{
		...
		"",
		"© 2026 StreamWeave · This is an automated message.",
		"",
	}, "\n")
}
...
<tr><td style="background:#0b1020;padding:28px 36px"><table role="presentation" cellspacing="0" cellpadding="0"><tr><td style="vertical-align:middle"><img src="cid:platform-logo" width="52" height="52" alt="StreamWeave" style="display:block;border:0;border-radius:14px"></td><td style="padding-left:14px;vertical-align:middle;color:#ffffff"><div style="font-size:18px;font-weight:700;letter-spacing:-.2px">StreamWeave</div><div style="font-size:12px;color:#9eeeff;letter-spacing:1.5px;text-transform:uppercase">Commerce Platform</div></td></tr></table></td></tr>
...
<tr><td style="padding:24px 36px 30px;border-top:1px solid #eef2f7;color:#64748b;font-size:12px;line-height:1.7">Need help? Contact <a href="mailto:` + template.HTMLEscapeString(supportEmail) + `" style="color:#0891b2;text-decoration:none">` + template.HTMLEscapeString(supportEmail) + `</a>.<br>This is an automated security message from StreamWeave. Please do not reply.<br><span style="color:#94a3b8">© 2026 StreamWeave</span></td></tr>
